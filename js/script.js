...
const pages = {
    ...
    "about.html": "./exam.html",
    "exam.html": "./notice.html",
    "notice.html": "./contact.html",
...
const prevPages = {
    ...
    "exam.html": "./about.html",
    "notice.html": "./exam.html",
    "contact.html": "./notice.html",
    "book.html": "./contact.html",
    "index.html": "./book.html"
};

// ডিফল্ট স্ক্রল মেমোরি অফ করা
...
function getCurrentPageName() {
    ...
    if (!page || page === "" || page.indexOf('.') === -1) {
        return "index.html";
    }
    return page;
}

// পেজ পরিবর্তন করার মেইন ফাংশন
function navigate(targetPage) {
    if (isRedirecting || !targetPage) return;
    isRedirecting = true;
    window.location.href = targetPage;
}

// ১. মাউসের চাকা (Wheel) দিয়ে স্ক্রল ডিটেক্ট করা
window.addEventListener("wheel", (event) => {
    ...
    // নিচে স্ক্রল করলে → পরের পেজে যাবে
    if (event.deltaY > 0 && (windowHeight + currentScrollY >= documentHeight - 5)) {
        navigate(pages[currentPage]);
    }

    // উপরে স্ক্রল করলে → আগের পেজে যাবে
    if (event.deltaY < 0 && currentScrollY <= 0) {
        navigate(prevPages[currentPage]);
    }
}, { passive: true });

// ২. Scrollbar Drag করলে
window.addEventListener("scroll", () => {
    if (isRedirecting) return;
    ...
    // একদম নিচে গেলে
    if (windowHeight + currentScrollY >= documentHeight - 2) {
        navigate(pages[currentPage]);
    }
